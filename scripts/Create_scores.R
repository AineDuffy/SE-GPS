...
### split unique gene-phenotypes into 5 CV 20 sets. in test set match all gene-phenotypes and so train will be rest.
...
Mixedmodel_all<-do.call(rbind, lapply(c(paste0('CVsample',rep(1:5))), function(CVsample){
  CVdataset=fread(paste0(scorefolder,dataset,'_se_dataset_sampled20_',CVsample,'.txt.gz'),data.table=F) #80% training Open target dataset
  CVdataset$ID=paste0(CVdataset$drugname,'-',CVdataset$gene,'-',CVdataset$parentterm)
  dataset_sampled80=subset(opentarget_fulldataset, !(ID %in% CVdataset$ID))#80% training drug dataset
...
lapply(c(paste0('CVsample',rep(1:5))), function(CVsample){
  ...
  ## multiply betas from mixedmodel with values from 20% dataset
...
## Create scores with predictors 
Combine_genescores<-lapply(paste0('CVsample', seq(1:5)), function(CVsample){
  CVdataset=fread(paste0(scorefolder,dataset,'_se_dataset_sampled20_',CVsample,'.txt.gz'),data.table=F) #20% test Open target dataset
  genescorefile=fread(paste0(scorefolder, 'Genescore_sum_across_predictor_',dataset,'_outcome_',outcome,'_',CVsample,'.txt'), data.table=F)
  Genescore_sumfile_20_drugs<-inner_join(CVdataset[c('drugname','gene','parentterm','category','category_full','mi','se','senomi_phase4')], genescorefile ) %>% distinct()
...
## Create scores without predictors 
Combine_genescores_nopredictors<-lapply(paste0('CVsample', seq(1:5)), function(CVsample){
  CVdataset=fread(paste0(scorefolder,dataset,'_se_dataset_sampled20_',CVsample,'.txt.gz'),data.table=F) #20% test Open target dataset
  genescorefile=fread(paste0(scorefolder, 'Genescore_sum_across_predictor_',dataset,'_outcome_',outcome,'_',CVsample,'.txt'), data.table=F)
  Genescore_sumfile_20_nopred<-genescorefile %>% distinct(gene,parentterm,genescoresum )
  Genescore_sumfile_20_drugs<-inner_join(CVdataset[c('drugname','gene','parentterm','category','category_full','mi','se','senomi_phase4')], Genescore_sumfile_20_nopred ) %>% distinct()
  Genescore_sumfile_20_drugs$CV=CVsample
  return(Genescore_sumfile_20_drugs)
})
Combine_genescores_nopredictors1<-do.call(rbind.fill,Combine_genescores_nopredictors)
write.table(Combine_genescores_nopredictors1, gzfile(paste0(scorefolder, 'All_genescoresum_',dataset,'_outcome_',outcome,'_sideeffectproject.txt.gz')), sep='\t',quote=F,row.names=F)
...
lapply(c('Onsides'), function(valdataset){
  #Calculate SE-GPS in OnSIDES dataset dataset using the OT weights which gave the max OR 
  ...
  ## multiply betas from mixedmodel with values from OnSIDES dataset
  ...
  Genescores_beta_genetic_genept=Genescores_beta_genetic %>% distinct(gene, parentterm, genescoresum) 
  genescorefile_drugs=inner_join(Validation_dataset[c('drugname','gene','parentterm','category','mi','se','senomi_phase4')],Genescores_beta_genetic_genept, by=c('gene', 'parentterm'))
  write.table(genescorefile_drugs, paste0(scorefolder, 'All_genescoresum_',valdataset,'_outcome_',outcome,'_sideeffectproject.txt.gz'), sep='\t', row.names=F, quote=F )
  print(dim(genescorefile_drugs))
...
lapply(c('Allgenes'), function(valdataset){
  ...
  # phenotype_allgene_pt2= phenotype_allgene_pt2 %>% filter(!category_full %in% c('Neoplasms','Neonatal','Pregnancy'))
  ...
  ## multiply betas from mixedmodel with values from Allgenes dataset
  ...
  print(head(Genescores_beta_genetic_drugs))
  write.table(Genescores_beta_genetic_drugs, paste0(scorefolder, 'All_genescoresum_across_all_predictors_',valdataset,'_outcome_',outcome,'_sideeffectproject.txt.gz'), sep='\t', row.names=F, quote=F )
...
##############################################################################
# 6. Create a severe drug dataset using boxed warnings and withdrawn drugs
##############################################################################
...
lapply(c('Onsides','Opentargets'), function(dataset){
  ...
  if (dataset == 'Opentargets'){
    #Open target warning dataset mapped to toxicity classes - map these to phecode categories
  ...
  lapply(c('senomi_phase4'), function(outcome){
    ...
    # Format DF - Create a data frame with  drugs and  unique side effects
