...
Combine_genescores_nopredictors<-lapply(paste0('CVsample', seq(1:5)), function(CVsample){
  CVdataset=fread(paste0(scorefolder,dataset,'_geneticdataset_',CVsample,'_DOE_matchmechanism.txt.gz'),data.table=F) #20% training Open target dataset
  genescorefile=fread(paste0(scorefolder, 'Genescore_sum_across_predictor_',dataset,'_outcome_',outcome,'_',CVsample,'_DOE.txt'), data.table=F)
  Genescore_sumfile_20_nopred<-genescorefile %>% distinct(gene,parentterm,genescoresum, moa )
  Genescore_sumfile_20_drugs<-inner_join(CVdataset[c('drugname','gene','parentterm','moa','category','category_full','mi','se','senomi_phase4')], Genescore_sumfile_20_nopred,relationship = "many-to-many" ) %>% distinct()
  Genescore_sumfile_20_drugs$CV=CVsample
  return(Genescore_sumfile_20_drugs)
})
Combine_genescores_nopredictors1<-do.call(rbind.fill,Combine_genescores_nopredictors)
write.table(Combine_genescores_nopredictors1, gzfile(paste0(scorefolder, 'All_genescoresum_',dataset,'_outcome_',outcome,'_sideeffectproject_DOE.txt.gz')), sep='\t',quote=F,row.names=F)
...
lapply(c('Onsides'), function(valdataset){
  #Calculate the  SE-GPS in OnSIDES dataset using the OT mixedmodel weights which gave the max OR 
...
lapply(c('Allgenes'), function(valdataset){
  ...
  # Calculate SE-GPS in all genes dataset (19000 +genes) using the OT mixedmodel weights which gave the max OR 
  OT_CV=fread(paste0(tables_dir, 'Table_S4_associations_across_CVtest_sets_DOE.txt'), data.table=F )
  samplecv=head(OT_CV[1],1)[[1]]
  mixedmodelresults=fread(paste0(scorefolder, '/Mixedeffect_weighted_model_ADR_severity_',dataset,'_outcome_',outcome,'_',samplecv,'_DOE_matchmechanism.txt'), data.table=F)
  mixedmodelresults=mixedmodelresults[!grepl('category|Intercept|moainhibitor', mixedmodelresults$Predictor),]
  ...
  #Gain of function predictons
  DOEGOF=fread(paste0('Data/Predictors_with_direction_effect_sideeffect_project_gof_predvalue.txt'),data.table=F)
  DOEGOF1=as.data.frame(DOEGOF %>% mutate(parentterm=as.character(trunc(as.numeric(Phecode)))) %>% group_by( gene, parentterm,category_full) %>% dplyr::summarise(omim_predicted:=max(omim_predicted),eqtl_Pheno_DOE:=max(eqtl_Pheno_DOE)
                                                                                                                                                                  ,hgmd_predicted:=max(hgmd_predicted),genebass_predicted:=max(genebass_predicted), OTgeneburden_doe:=max(OTgeneburden_doe),
  ...
  ## Make Clinical Variant predictor
  doe_both$clinicalvariant_DOE=0
  doe_both$clinicalvariant_DOE[rowSums(doe_both[, clincicalpred_DOE]) == 1]<-1
  doe_both$clinicalvariant_DOE[rowSums(doe_both[, clincicalpred_DOE]) == 2]<-2
  doe_both$clinicalvariant_DOE[rowSums(doe_both[, clincicalpred_DOE]) == 3]<-3
  doe_both$clinicalvariant_DOE[rowSums(doe_both[, clincicalpred_DOE]) == 0]<-0
  ...
  doe_both$geneburden_doe=0
  doe_both$geneburden_doe[rowSums(doe_both[, geneburden_doe]) != 0]<-1
  
  singlevar_doe=c('ravar_snp_predicted','genebass_predicted')
  doe_both$singlevar_doe=0
  doe_both$singlevar_doe[rowSums(doe_both[, singlevar_doe]) > 0]<-1
  doe_both$singlevar_doe[rowSums(doe_both[, singlevar_doe]) < 0]<--1
  
  gwastrait_doe=c('OTG_DOE','eqtl_Pheno_DOE')
  doe_both$gwastrait_doe=0
  doe_both$gwastrait_doe[rowSums(doe_both[, gwastrait_doe]) > 0]<-1
  doe_both$gwastrait_doe[rowSums(doe_both[, gwastrait_doe]) < 0]<--1
  ...
  phecodef = phecodef %>% rename(category_full=category) %>% mutate(category=gsub('_.*', '',phecode), phecode=gsub('.*_', '',phecode)) %>%
